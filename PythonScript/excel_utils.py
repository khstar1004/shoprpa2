...
def finalize_dataframe_for_excel(df: pd.DataFrame) -> pd.DataFrame:
    """Finalize DataFrame for Excel output with proper formatting."""
    logging.info(f"Finalizing DataFrame for Excel. Input shape: {df.shape}")
    
    # Create a copy to avoid modifying the original
    df = df.copy()
    
    # Standardize column names - remove extra spaces and normalize
    ...
    # Convert price columns to numeric with proper error handling
    price_columns = [
        '판매단가(V포함)',
        '판매가(V포함)(2)',
        '판매단가(V포함)(2)',
    ...
    logging.info(f"DataFrame finalized. Output shape: {df.shape}")
    return df
